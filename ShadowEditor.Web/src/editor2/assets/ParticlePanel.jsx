...
class ParticlePanel extends React.Component {
    ...
    render() {
        ...
        return <div className={classNames('ParticlePanel', className)} style={style}>
            <SearchField
                data={categoryData}
    ...
    update() {
        fetch(`/api/Category/List?type=Particle`).then(response => {
        ...
        fetch(`/api/Particle/List`).then(response => {
            response.json().then(obj => {
                this.data = obj.Data;
                this.setState({
                    data: this.data,
    ...
    handleDelete(data) {
        ...
        app.confirm({
            ...
            onOK: () => {
                fetch(`${server}/api/Particle/Delete?ID=${data.id}`, {
                    method: 'POST',
                }).then(response => {
                    response.json().then(obj => {
                        if (obj.Code === 200) {
