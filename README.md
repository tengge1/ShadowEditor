...
6. 注册用户：输入用户名`cs`、密码`123`、确认密码`123`、姓名`测试`，点击确定，提示注册成功。使用`cs`、`123`可以成功登录系统。管理员登录，查看角色是`用户`，正确。
7. 修改密码：管理员登录，原密码`123456`，新密码`123`,确认密码`123`，修改成功。使用`admin`和`123`可以正常登录。
8. 还原以前删除的`C#`版`THREE.js`项目。
9. 禁用`Session`，避免第一次打开网页时`会话状态已创建一个会话 ID，但由于响应已被应用程序刷新而无法保存它。`的报错。
10. 管理员组默认具有所有权限。
...
23. 初始化不再生成游客角色。
24. 删除整理贴图、整理模型、整理缩略图功能。
25. 暂时禁用权限管理，有需要的可在`Web.config`中打开。
26. 点击场景中的物体，场景树状图上展开该节点和它的所有父节点，并将该节点滚动到可视范围内。
27. 修复点击场景中的物体，场景中的物体不高亮bug。
28. 新建三维园区。
29. 工具栏改为横向，并进行分类。
30. 禁用鼠标放到物体上高亮。
31. 修复点击眼睛图标时，整个场景树折叠bug。
32. 新增添加模型模式：添加到中心、点击场景添加。
33. 增加显示选项，设置选中边框颜色和粗细。
