...
1. 修复属性面板修改名称，场景树状图无法及时响应的bug。
2. 修复属性面板修改名称，无法记住树节点展开状态的bug。
3. 物体名称超过8个文字后，将显示省略号。鼠标移到节点上，显示完整名称。
4. 新增全屏播放功能。
...
8. 修复每帧连续绘制场景两次bug。
9. 物体选中效果优化：等宽描边。
10. 修复整理模型工具，复制模型时未复制文件夹中所有文件的bug。
11. 所有设置改为存储在`localStorage`中，不再保存在场景配置中。
12. 重构平行光帮助器、半球光帮助器、点光源帮助器、矩形光帮助器、聚光灯帮助器，采用事件驱动方式，不再对编辑器过度耦合。
