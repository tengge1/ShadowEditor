...
class SearchField extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={classNames('SearchField', className)} onClick={this.stopPropagation}>
        {showAddButton && <IconButton icon={'add'} onClick={this.handleAdd} />}
        <input className={'input'} style={style} placeholder={placeholder} value={value} onChange={this.handleChange} onInput={this.handleInput} onKeyDown={this.handleKeyDown} />
        <IconButton icon={'close'} onClick={this.handleReset} />
        {showFilterButton && <IconButton icon={'filter'} className={classNames(filterShow && 'selected')} onClick={this.handleShowFilter} />}
        {showFilterButton && (
          <div className={classNames('category', !filterShow && 'hidden')}>
            <div className={'item'} key={''}>
              <CheckBox name={''} checked={categories.indexOf('') > -1} onChange={this.handleCheckBoxChange} />
              <label className={'title'}>{_t('No Type')}</label>
            </div>
            {data.map(n => {
              return (
                <div className={'item'} key={n.ID}>
                  <CheckBox name={n.ID} checked={categories.indexOf(n.ID) > -1} onChange={this.handleCheckBoxChange} />
