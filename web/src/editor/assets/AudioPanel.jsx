...
class AudioPanel extends React.Component {
    ...
    update() {
        fetch(`${global.app.options.server}/api/Category/List?Type=Audio`).then(response => {
        ...
        fetch(`${global.app.options.server}/api/Audio/List`).then(response => {
            response.json().then(obj => {
                if (obj.Code !== 200) {
                    global.app.toast(_t(obj.Msg), 'warn');
                    return;
    ...
    handleClick(data) {
        global.app.call(`selectAudio`, this, data);
    }

    // ------------------------------- 上传 ---------------------------------------

    handleAdd() {
        global.app.upload(`${global.app.options.server}/api/Audio/Add`, obj => {
            if (obj.Code === 200) {
                this.update();
            }
    ...
    handleEdit(data) {
        var win = global.app.createElement(EditWindow, {
            ...
            typeName: _t('Audio'),
            data,
            saveUrl: `${global.app.options.server}/api/Audio/Edit`,
    ...
    handleDelete(data) {
        global.app.confirm({
            ...
            onOK: () => {
                fetch(`${global.app.options.server}/api/Audio/Delete?ID=${data.id}`, {
                    method: 'POST'
                }).then(response => {
                    response.json().then(obj => {
                        if (obj.Code !== 200) {
