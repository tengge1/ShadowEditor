...
class ScreenshotPanel extends React.Component {
    ...
    update() {
        fetch(`${global.app.options.server}/api/Category/List?Type=Screenshot`).then(response => {
        ...
        fetch(`${global.app.options.server}/api/Screenshot/List`).then(response => {
            response.json().then(obj => {
                if (obj.Code !== 200) {
                    global.app.toast(_t(obj.Msg), 'warn');
                    return;
    ...
    handleEdit(data) {
        var win = global.app.createElement(EditWindow, {
            ...
            data,
            saveUrl: `${global.app.options.server}/api/Screenshot/Edit`,
    ...
    handleDelete(data) {
        global.app.confirm({
            ...
            onOK: () => {
                fetch(`${global.app.options.server}/api/Screenshot/Delete?ID=${data.id}`, {
                    method: 'POST'
                }).then(response => {
                    response.json().then(obj => {
                        if (obj.Code !== 200) {
