...
class ModelPanel extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={classNames('ModelPanel', className)} style={style}>
        <SearchField data={categoryData} placeholder={_t('Search Content')} showAddButton={!enableAuthority || authorities.includes('ADD_MESH')} showFilterButton onAdd={this.handleAdd} onInput={this.handleSearch.bind(this)} />
        <ImageList data={imageListData} showEditButton={!enableAuthority || authorities.includes('EDIT_MESH')} showDeleteButton={!enableAuthority || authorities.includes('DELETE_MESH')} onClick={this.handleClick} onEdit={this.handleEdit} onDelete={this.handleDelete} />
      </div>
    );
  }
  ...
  clickSceneToAdd(obj) {
    ...
    global.app.on(`gpuPick.ModelPanel`, intersect => {
      // 鼠标移动出现预览效果
      if (!intersect.point) {
        return;
      }
    ...
    global.app.on(`raycast.ModelPanel`, intersect => {
      // 点击鼠标放置模型
