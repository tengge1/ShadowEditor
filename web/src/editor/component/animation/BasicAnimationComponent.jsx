...
class BasicAnimationComponent extends React.Component {
  ...
  render() {
    ...
    return (
      <PropertyGroup title={_t('Basic Information')} show={show} expanded={expanded} onExpand={this.handleExpand}>
        <TextProperty label={_t('Name')} name={'name'} value={name} onChange={this.handleChange} />
        <DisplayProperty label={_t('Target')} name={'target'} value={target ? target : `(${_t('None')})`} btnText={_t('Set Target')} btnShow={global.app.editor.selected !== null} onClick={this.handleSetTarget} onChange={this.handleChange} />
        <SelectProperty label={_t('Type')} options={this.animationType} name={'type'} value={type} onChange={this.handleChange} />
        <NumberProperty label={_t('BeginTime')} name={'beginTime'} value={beginTime} onChange={this.handleChange} />
        <NumberProperty label={_t('EndTime')} name={'endTime'} value={endTime} onChange={this.handleChange} />
      </PropertyGroup>
    );
  }
  ...
  handleUpdate(animation) {
    ...
    // 将当前选中的动画设置到全局中
    global.app.editor.selectAnimation = animation;
