...
import TweenHelps from '../helper/animation/TweenHelpers';
...
class Editor extends React.Component {
  constructor(props) {
    ...
    // 实例化补间动画辅助器
    this.tweenHelps = new TweenHelps();
    ...
    return (
      <>
        <BorderLayout className={'Editor'}>
          ...
          {<EditorStatusBar region={'south'} show={statusBarShow} />}
          {<AssetsPanel region={'west'} split show={assetsPanelShow} onToggle={this.onToggle} />}
          {isLogin && <EditorSideBar region={'east'} split show={sidebarShow} onToggle={this.onToggle} />}
          <BorderLayout region={'center'}>
            {isLogin && <EditorToolbar region={'north'} show={toolbarShow} />}
            <Viewport region={'center'} />
            {isLogin && <TimelinePanel region={'south'} split show={timelinePanelShow} onToggle={this.onToggle} />}
          </BorderLayout>
        </BorderLayout>
        {elements.map((n, i) => {
  ...
  componentDidMount() {
    ...
    // 监听新场景加载
    global.app.on('sceneLoaded.Editor', () => {
      this.animations.forEach(item => {
        if (item.animations.length !== 0) {
          item.animations.forEach(childItem => {
            this.tweenHelps.addAnimation(childItem);
          });
        }
      });
    });
    ...
    // 相机
    this.DEFAULT_CAMERA = new THREE.PerspectiveCamera(50, width / height, 0.1, 100000);
