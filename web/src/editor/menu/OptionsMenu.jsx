...
class OptionsMenu extends React.Component {
  ...
  render() {
    ...
    return (
      <MenuItem title={_t('Options')}>
        <MenuItem title={_t('Display')} onClick={this.handleDisplayOptions} />
        <MenuItem title={_t('Renderer')} onClick={this.handleRendererOptions} />
        <MenuItem title={_t('Helpers')} onClick={this.handleHelperOptions} />
        <MenuItem title={_t('Filter')} onClick={this.handleFilterOptions} />
        <MenuItem title={_t('Weather')} onClick={this.handleWeatherOptions} />
        <MenuItemSeparator />
        {isLogin && (
          <MenuItem title={_t('Control Mode')}>
            <MenuItem name={'EditorControls'} title={_t('Editor Controls')} selected={controlMode === 'EditorControls'} onClick={this.handleChangeControlMode} />
            <MenuItem name={'FreeControls'} title={_t('Free Controls')} selected={controlMode === 'FreeControls'} onClick={this.handleChangeControlMode} />
          </MenuItem>
        )}
        {isLogin && (
          <MenuItem title={_t('Select Mode')}>
            <MenuItem name={'whole'} title={_t('Select Whole')} selected={selectMode === 'whole'} onClick={this.handleChangeSelectMode} />
            <MenuItem name={'part'} title={_t('Select Part')} selected={selectMode === 'part'} onClick={this.handleChangeSelectMode} />
          </MenuItem>
        )}
        {isLogin && (
          <MenuItem title={_t('Add Mode')}>
            <MenuItem name={'center'} title={_t('Add To Center')} selected={addMode === 'center'} onClick={this.handleChangeAddMode} />
            <MenuItem name={'click'} title={_t('Click Scene To Add')} selected={addMode === 'click'} onClick={this.handleChangeAddMode} />
          </MenuItem>
        )}
        <MenuItemSeparator />
        <MenuItem title={_t('Language')}>
          <MenuItem title={'English'} selected={lang === 'en-US'} onClick={this.handleChangeEnglish} />
          <MenuItem title={'简体中文'} selected={lang === 'zh-CN'} onClick={this.handleChangeChinese} />
          <MenuItem title={'繁體中文'} selected={lang === 'zh-TW'} onClick={this.handleChangeTraditionalChinese} />
          <MenuItem title={'日本語'} selected={lang === 'ja-JP'} onClick={this.handleChangeJapanese} />
          <MenuItem title={'한국어'} selected={lang === 'ko-KR'} onClick={this.handleChangeKorean} />
          <MenuItem title={'русский'} selected={lang === 'ru-RU'} onClick={this.handleChangeRussian} />
          <MenuItem title={'Le français'} selected={lang === 'fr-FR'} onClick={this.handleChangeFrench} />
        </MenuItem>
        {isLogin && (
          <MenuItem title={_t('Debug Mode')}>
            <MenuItem title={_t('Enable')} selected={global.app.debug} onClick={this.handleEnableDebugMode} />
            <MenuItem title={_t('Disable')} selected={!global.app.debug} onClick={this.handleDisableDebugMode} />
          </MenuItem>
        )}
      </MenuItem>
    );
