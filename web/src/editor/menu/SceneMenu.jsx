...
class SceneMenu extends React.Component {
  ...
  render() {
    ...
    return (
      <MenuItem title={_t('Scene')}>
        {!enableAuthority || isLogin ? (
          <MenuItem title={_t('New')}>
            <MenuItem title={_t('Empty Scene')} onClick={this.handleCreateEmptyScene} />
            <MenuItem title={_t('District (Test)')} onClick={this.handleCreateDistrictAndIndoor} />
          </MenuItem>
        ) : null}
        {!enableAuthority || authorities.includes('SAVE_SCENE') ? <MenuItem title={_t('Save')} onClick={this.handleSaveScene} /> : null}
        {!enableAuthority || authorities.includes('SAVE_SCENE') ? <MenuItem title={_t('Save As')} onClick={this.handleSaveAsScene} /> : null}
        {!enableAuthority || authorities.includes('SAVE_SCENE') ? <MenuItemSeparator /> : null}
        {!enableAuthority || isLogin ? (
          <MenuItem title={_t('Export Scene')}>
            <MenuItem title={_t('To JSON File')} onClick={this.handleExportSceneToJson} />
            <MenuItem title={_t('To Collada File')} onClick={this.handleExportSceneToCollada} />
            <MenuItem title={_t('To GLTF File')} onClick={this.handleExportSceneToGltf} />
            <MenuItem title={_t('To OBJ File')} onClick={this.handleExportSceneToOBJ} />
            <MenuItem title={_t('To PLY File')} onClick={this.handleExportSceneToPLY} />
            <MenuItem title={_t('To STL File')} onClick={this.handleExportSceneToSTL} />
          </MenuItem>
        ) : null}
        {!enableAuthority || authorities.includes('PUBLISH_SCENE') ? <MenuItem title={_t('Publish Scene')} onClick={this.handlePublishScene} /> : null}
      </MenuItem>
    );
  }
  ...
  componentDidMount() {
    if (global.app.debug) {
      ...
      global.app.on(`appStarted.dev-3d-gis`, () => {
        setTimeout(() => {
          this.handleCreateGISScene && this.handleCreateGISScene();
        });
      });
    }
  }
  ...
  handleExportSceneToPLY() {
    this.querySceneName().then(name => {
      global.app.require('PLYExporter').then(() => {
        ...
        StringUtils.saveString(
          exporter.parse(global.app.editor.scene, {
