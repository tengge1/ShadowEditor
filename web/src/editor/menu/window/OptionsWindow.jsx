...
class OptionsWindow extends React.Component {
  ...
  render() {
    ...
    return (
      <Window className={'OptionsWindow'} title={_t('Settings')} style={{ width: '300px', height: '400px' }} mask={false} onClose={this.handleClose}>
        <Content>
          <TabLayout className={'tab'} activeTabIndex={activeTabIndex} onActiveTabChange={this.handleActiveTabChange}>
            <DisplayPanel title={_t('Display')} ref={this.displayRef} />
            <RendererPanel title={_t('Renderer')} ref={this.rendererRef} />
            <HelperPanel title={_t('Helpers')} ref={this.helperRef} />
            <FilterPanel title={_t('Filter')} ref={this.filterRef} />
            <WeatherPanel title={_t('Weather')} ref={this.weatherRef} />
          </TabLayout>
        </Content>
        <Buttons>
